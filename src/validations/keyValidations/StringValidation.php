    ...
    protected function checkOptions()
    {
        ...
        if (!is_null($this->getOption(Properties::ONE_OF))) {
            foreach ($this->getOption(Properties::ONE_OF) as $s) {
                if (!is_string($s)) {
                    trigger_error(sprintf('%s must contain strings, %s found', Properties::ONE_OF, gettype($s)),
                        E_USER_ERROR);
                }
            }
        }
        ...
        // TODO check that starWith and endWith are not in vialation with lengt, minLength, maxLength, trimmed and oneOf
    }
}